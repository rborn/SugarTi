...
function getProfiles(id, callback) {
	...
	fs.readdir(PROFILES_DIR, function(err, files) {
		...
		if (!profiles.length) {
			...
			var msg = 'One matching profile detected, trying with this one:\n\n\t'.green + profiles[0].name.white.bold + ' with id: '.grey + profiles[0].id.white.bold + ' and Developer name: '.grey + profiles[0].developer_name.white.bold;
			utils.message(msg);

			callback(null, profiles[0].profile_file, profiles[0].developer_name);
		}
